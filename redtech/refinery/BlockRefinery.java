...
public class BlockRefinery extends BlockContainer {
	...
	private Icon RefineryIconTop;

	public BlockRefinery(int id, boolean isActive) {
	...
	public void registerIcons(IconRegister iconRegister) {
		this.blockIcon = iconRegister.registerIcon(RedTech.modid + ":"
				+ (this.isActive ? "refinery_side_lit" : "refinery_side_idle"));
		this.RefineryIconTop = iconRegister
				.registerIcon(RedTech.modid
						+ ":"
						+ (this.isActive ? "refinery_front_lit"
								: "refinery_front_idle"));
	}
	...
	public Icon getIcon(int par1, int par2) {
		return par1 == 1 ? this.blockIcon : (par1 == 0 ? this.blockIcon
				: (par1 != par2 ? this.blockIcon : this.RefineryIconTop));
	}

	@Override
	public int idDropped(int par1, Random par2Random, int par3) {
		return RedTech.blockRefineryIdle.blockID;
	}
	...
	public boolean onBlockActivated(World world, int x, int y, int z,
		...
		if (!world.isRemote) {
			FMLNetworkHandler.openGui(player, RedTech.instance,
					RedTech.guiIdRefinery, world, x, y, z);
		}
	...
	public static void updateFurnaceBlockState(boolean par0, World par1World,
		...
		if (par0) {
			par1World.setBlock(par2, par3, par4,
					RedTech.blockRefineryActive.blockID);
		} else {
			par1World.setBlock(par2, par3, par4,
					RedTech.blockRefineryIdle.blockID);
		}
	...
	public void breakBlock(World par1World, int par2, int par3, int par4,
		...
		if (!keepRefineryInventory) {
			TileEntityRefinery TileEntityRefinery = (TileEntityRefinery) par1World
					.getBlockTileEntity(par2, par3, par4);
				...
				for (int j1 = 0; j1 < TileEntityRefinery.getSizeInventory(); ++j1) {
					...
					if (itemstack != null) {
						...
						while (itemstack.stackSize > 0) {
							...
							float f3 = 0.05F;
							entityitem.motionX = (float) this.RefineryRand
									.nextGaussian() * f3;
							entityitem.motionY = (float) this.RefineryRand
									.nextGaussian() * f3 + 0.2F;
							entityitem.motionZ = (float) this.RefineryRand
									.nextGaussian() * f3;
